    ...
    <div className="flex flex-col min-h-screen">
      <header className="flex items-center justify-between px-9 py-5 bg-white border-b border-gray-100">
        <button onClick={() => router.push("/dashboard/empleos")} className="flex items-center gap-2 text-gris text-sm">
          ← Empleos / <span className="text-tinta font-semibold">{v.titulo}</span>
        </button>
      </header>
        ...
        <div className="col-span-2 space-y-5">
          <div className="rounded-2xl bg-white border border-gray-100 p-6">
            ...
            <div className="grid grid-cols-2 gap-2 text-[14px]">
              {v.cumple.map((s) => (
                <div key={s} className="flex items-center gap-2"><span className="text-teal">✓</span> {s}</div>
              ))}
              {v.faltantes.map((s) => (
                <div key={s} className="flex items-center gap-2"><span className="text-rojo">✕</span> {s} (te falta)</div>
              ))}
            </div>
          </div>
          {v.faltantes.length > 0 && (
            <div className="rounded-2xl bg-tealT p-5 text-teal2 text-[15px] flex gap-2">
              <span>✦</span>
        ...
        <div className="space-y-4">
          <div className="rounded-2xl bg-rojoT p-5">
            ...
            <div className="flex flex-wrap gap-2 mb-2">
              ...
              )) : <span className="text-sm text-teal2">¡Cumples todos los requisitos! 🎉</span>}
            </div>
          </div>
